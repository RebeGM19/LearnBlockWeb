...
LearnBlock.Events.Change.prototype.run = function (a) {
    ...
    if (b) switch (b.mutator && b.mutator.setVisible(!1), a = a ? this.newValue : this.oldValue, this.element) {
        ...
        case "mutation":
            var c =
                "";
            b.mutationToDom && (c = (c = b.mutationToDom()) && LearnBlock.Xml.domToText(c));
            if (b.domToMutation) {
                var d = LearnBlock.Xml.textToDom(a || "<mutation/>");
                b.domToMutation(d)
            }
            LearnBlock.Events.fire(new LearnBlock.Events.Change(b, "mutation", null, c, a));
            break;
        default:
...
LearnBlock.Xml.blockToDom = function (a, b) {
    var c = LearnBlock.utils.xml.createElement(a.isShadow() ? "shadow" : "block");
...
LearnBlock.InsertionMarkerManager.prototype.wouldDeleteBlock = function () {
...
LearnBlock.InsertionMarkerManager.prototype.update = function (a, b) {
    ...
    if ((this.wouldDeleteBlock_ = this.shouldDelete_(c, b)) || this.shouldUpdatePreviews_(c, a)) LearnBlock.Events.disable(), this.maybeHidePreview_(c), this.maybeShowPreview_(c), LearnBlock.Events.enable()
};
...
LearnBlock.InsertionMarkerManager.prototype.shouldDelete_ = function (a, b) {
    var c = a && !!a.closest && b != LearnBlock.DELETE_AREA_TOOLBOX;
...
LearnBlock.ScrollbarPair = function (a) {
    ...
    LearnBlock.utils.dom.insertAfter(this.corner_, a.getBubbleCanvas())
};
...
LearnBlock.Tooltip.LIMIT = 50;
LearnBlock.Tooltip.mouseOutPid_ = 0;
LearnBlock.Tooltip.showPid_ = 0;
LearnBlock.Tooltip.lastX_ = 0;
LearnBlock.Tooltip.lastY_ = 0;
LearnBlock.Tooltip.element_ = null;
LearnBlock.Tooltip.poisonedElement_ = null;
LearnBlock.Tooltip.OFFSET_X = 0;
LearnBlock.Tooltip.OFFSET_Y = 10;
LearnBlock.Tooltip.RADIUS_OK = 10;
LearnBlock.Tooltip.HOVER_MS = 750;
LearnBlock.Tooltip.MARGINS = 5;
LearnBlock.Tooltip.DIV = null;
LearnBlock.Tooltip.createDom = function () {
    LearnBlock.Tooltip.DIV || (LearnBlock.Tooltip.DIV = document.createElement("div"), LearnBlock.Tooltip.DIV.className = "blocklyTooltipDiv", document.body.appendChild(LearnBlock.Tooltip.DIV))
};
LearnBlock.Tooltip.bindMouseEvents = function (a) {
    LearnBlock.bindEvent_(a, "mouseover", null, LearnBlock.Tooltip.onMouseOver_);
    LearnBlock.bindEvent_(a, "mouseout", null, LearnBlock.Tooltip.onMouseOut_);
    a.addEventListener("mousemove", LearnBlock.Tooltip.onMouseMove_, !1)
};
LearnBlock.Tooltip.onMouseOver_ = function (a) {
    if (!LearnBlock.Tooltip.blocked_) {
        for (a = a.currentTarget;
            "string" != typeof a.tooltip && "function" != typeof a.tooltip;) a = a.tooltip;
        LearnBlock.Tooltip.element_ != a && (LearnBlock.Tooltip.hide(), LearnBlock.Tooltip.poisonedElement_ = null, LearnBlock.Tooltip.element_ = a);
        clearTimeout(LearnBlock.Tooltip.mouseOutPid_)
    }
};
LearnBlock.Tooltip.onMouseOut_ = function (a) {
    LearnBlock.Tooltip.blocked_ || (LearnBlock.Tooltip.mouseOutPid_ = setTimeout(function () {
        LearnBlock.Tooltip.element_ = null;
        LearnBlock.Tooltip.poisonedElement_ = null;
        LearnBlock.Tooltip.hide()
    }, 1), clearTimeout(LearnBlock.Tooltip.showPid_))
};
LearnBlock.Tooltip.onMouseMove_ = function (a) {
    if (LearnBlock.Tooltip.element_ && LearnBlock.Tooltip.element_.tooltip && !LearnBlock.Tooltip.blocked_)
        if (LearnBlock.Tooltip.visible) {
            var b = LearnBlock.Tooltip.lastX_ - a.pageX;
            a = LearnBlock.Tooltip.lastY_ - a.pageY;
            Math.sqrt(b * b + a * a) > LearnBlock.Tooltip.RADIUS_OK && LearnBlock.Tooltip.hide()
        } else LearnBlock.Tooltip.poisonedElement_ != LearnBlock.Tooltip.element_ && (clearTimeout(LearnBlock.Tooltip.showPid_), LearnBlock.Tooltip.lastX_ = a.pageX, LearnBlock.Tooltip.lastY_ = a.pageY, LearnBlock.Tooltip.showPid_ = setTimeout(LearnBlock.Tooltip.show_,
            LearnBlock.Tooltip.HOVER_MS))
};
LearnBlock.Tooltip.hide = function () {
    LearnBlock.Tooltip.visible && (LearnBlock.Tooltip.visible = !1, LearnBlock.Tooltip.DIV && (LearnBlock.Tooltip.DIV.style.display = "none"));
    LearnBlock.Tooltip.showPid_ && clearTimeout(LearnBlock.Tooltip.showPid_)
};
LearnBlock.Tooltip.block = function () {
    LearnBlock.Tooltip.hide();
    LearnBlock.Tooltip.blocked_ = !0
};
LearnBlock.Tooltip.unblock = function () {
    LearnBlock.Tooltip.blocked_ = !1
};
LearnBlock.Tooltip.show_ = function () {
    if (!LearnBlock.Tooltip.blocked_ && (LearnBlock.Tooltip.poisonedElement_ = LearnBlock.Tooltip.element_, LearnBlock.Tooltip.DIV)) {
        LearnBlock.Tooltip.DIV.innerHTML = "";
        for (var a = LearnBlock.Tooltip.element_.tooltip;
            "function" == typeof a;) a = a();
        a = LearnBlock.utils.string.wrap(a, LearnBlock.Tooltip.LIMIT);
        a = a.split("\n");
        for (var b = 0; b < a.length; b++) {
            var c = document.createElement("div");
            c.appendChild(document.createTextNode(a[b]));
            LearnBlock.Tooltip.DIV.appendChild(c)
        }
        a = LearnBlock.Tooltip.element_.RTL;
        b = document.documentElement.clientWidth;
        c = document.documentElement.clientHeight;
        LearnBlock.Tooltip.DIV.style.direction = a ? "rtl" : "ltr";
        LearnBlock.Tooltip.DIV.style.display = "block";
        LearnBlock.Tooltip.visible = !0;
        var d = LearnBlock.Tooltip.lastX_;
        d = a ? d - (LearnBlock.Tooltip.OFFSET_X + LearnBlock.Tooltip.DIV.offsetWidth) : d + LearnBlock.Tooltip.OFFSET_X;
        var e = LearnBlock.Tooltip.lastY_ + LearnBlock.Tooltip.OFFSET_Y;
        e + LearnBlock.Tooltip.DIV.offsetHeight > c + window.scrollY && (e -= LearnBlock.Tooltip.DIV.offsetHeight + 2 * LearnBlock.Tooltip.OFFSET_Y);
        a ? d = Math.max(LearnBlock.Tooltip.MARGINS - window.scrollX,
            d) : d + LearnBlock.Tooltip.DIV.offsetWidth > b + window.scrollX - 2 * LearnBlock.Tooltip.MARGINS && (d = b - LearnBlock.Tooltip.DIV.offsetWidth - 2 * LearnBlock.Tooltip.MARGINS);
        LearnBlock.Tooltip.DIV.style.top = e + "px";
...
LearnBlock.Block = function (a, b, c) {
    ...
    this.commentModel = {
        text: null,
        pinned: !1,
        size: new LearnBlock.utils.Size(160, 80)
    };
...
LearnBlock.Block.prototype.setHelpUrl = function (a) {
    this.helpUrl = a
};
...
LearnBlock.Block.prototype.getColourShadow = function () {
    var a = this.getColourSecondary();
    return a ? a : LearnBlock.utils.colour.blend("#fff", this.getColour(), .6)
};
...
LearnBlock.Block.prototype.jsonInit = function (a) {
    ...
    void 0 !== a.helpUrl && (c = a.helpUrl, c = LearnBlock.utils.replaceMessageReferences(c), this.setHelpUrl(c));
    ...
    void 0 !== a.mutator && LearnBlock.Extensions.apply(a.mutator, this, !0);
...
LearnBlock.RenderedConnection.prototype.respawnShadow_ = function () {
    var a = this.getSourceBlock(),
        b = this.getShadowDom();
    if (a.workspace && b && LearnBlock.Events.recordUndo) {
        LearnBlock.RenderedConnection.superClass_.respawnShadow_.call(this);
        b = this.targetBlock();
...
LearnBlock.BlockSvg = function (a, b, c) {
    ...
    LearnBlock.Tooltip.bindMouseEvents(this.svgPath_);
...
LearnBlock.WorkspaceSvg.prototype.isMutator = !1;
...
LearnBlock.WorkspaceSvg.prototype.createDom = function (a) {
    ...
    this.svgBubbleCanvas_ = LearnBlock.utils.dom.createSvgElement("g", {
...
LearnBlock.WorkspaceSvg.prototype.getBubbleCanvas = function () {
...
LearnBlock.WorkspaceSvg.prototype.traceOn = function () {
...
LearnBlock.WorkspaceSvg.prototype.pasteBlock_ = function (a) {
    ...
    LearnBlock.Events.isEnabled() && !b.isShadow() && LearnBlock.Events.fire(new LearnBlock.Events.BlockCreate(b));
    b.select()
};
LearnBlock.WorkspaceSvg.prototype.pasteWorkspaceComment_ = function (a) {
    LearnBlock.Events.disable();
    try {
        var b = LearnBlock.WorkspaceCommentSvg.fromXml(a, this),
...
LearnBlock.WorkspaceSvg.prototype.cleanUp = function () {
    ...
    for (var a = this.getTopBlocks(!0), b = 0, c = 0, d; d = a[c]; c++)
        if (d.isMovable()) {
            ...
            d.moveBy(-e.x, b - e.y);
            d.snapToGrid();
            b = d.getRelativeToSurfaceXY().y + d.getHeightWidth().height + LearnBlock.BlockSvg.MIN_BLOCK_Y
        } LearnBlock.Events.setGroup(!1);
...
LearnBlock.WorkspaceSvg.prototype.zoom = function (a, b, c) {
...
LearnBlock.WorkspaceSvg.prototype.zoomCenter = function (a) {
...
LearnBlock.WorkspaceSvg.prototype.zoomToFit = function () {
    if (this.isMovable()) {
        var a = this.getMetrics(),
            b = a.viewWidth;
        a = a.viewHeight;
...
LearnBlock.WorkspaceSvg.prototype.beginCanvasTransition = function () {
    ...
    LearnBlock.utils.dom.addClass(this.svgBubbleCanvas_, "blocklyCanvasTransitioning")
};
LearnBlock.WorkspaceSvg.prototype.endCanvasTransition = function () {
    ...
    LearnBlock.utils.dom.removeClass(this.svgBubbleCanvas_, "blocklyCanvasTransitioning")
};
LearnBlock.WorkspaceSvg.prototype.scrollCenter = function () {
    if (this.isMovable()) {
        var a = this.getMetrics(),
...
LearnBlock.WorkspaceSvg.prototype.centerOnBlock = function (a) {
    if (this.isMovable()) {
...
LearnBlock.WorkspaceSvg.prototype.getAudioManager = function () {
...
LearnBlock.Flyout.prototype.createRect_ = function (a, b, c, d, e) {
    ...
    b.tooltip = a;
    LearnBlock.Tooltip.bindMouseEvents(b);
